...
internal sealed class BackgroundJobDashboardStateStore : IBackgroundJobDashboardStateStore
{
    ...
    )
    {
        ...
        _definitions = jobs.Where(ShouldInclude).Select(CreateDefinition).ToDictionary(item => item.Alias, StringComparer.OrdinalIgnoreCase);

        _items = new ConcurrentDictionary<string, BackgroundJobDashboardItem>(
            _definitions.ToDictionary(x => x.Key, x => x.Value, StringComparer.OrdinalIgnoreCase)
        );
    }
    ...
    )
    {
        ...
        Update(
            ...
            item =>
            {
                ...
                item.LastDuration =
                    startedAt.HasValue ? completedAt - startedAt.Value
                    : item.LastStartedAt.HasValue ? completedAt - item.LastStartedAt.Value
                    : null;
    ...
    private void Update(IRecurringBackgroundJob job, Action<BackgroundJobDashboardItem> update)
    {
        ...
        _items.AddOrUpdate(
            ...
            _ =>
            {
                var item = _definitions.TryGetValue(alias, out BackgroundJobDashboardItem? definition) ? Clone(definition) : CreateDefinition(job);
                update(item);
                return item;
            },
            (_, existing) =>
    ...
    private void IncrementExecutionCount(string alias) => _activeExecutionCounts.AddOrUpdate(alias, 1, (_, existing) => existing + 1);
...
internal sealed class BackgroundJobDashboardService : IBackgroundJobDashboardService
{
    ...
    public IReadOnlyCollection<BackgroundJobDashboardItem> GetJobs()
    {
        var items = _stateStore
            .GetAll()
            .Select(item => new BackgroundJobDashboardItem
            {
                Alias = item.Alias,
                Name = item.Name,
                Type = item.Type,
